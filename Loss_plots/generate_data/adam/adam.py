...
class ADAM(Optimizer):
    ...
    def load_params(self, load_dir: str) -> None:
        ...
        t = t[1:-1]
        self._t = np.fromstring(t, dtype=np.int32, sep=' ')

    def minimize(self, objective_function: Callable[[np.ndarray], float], initial_point: np.ndarray,
                 gradient_function: Callable[[np.ndarray], float]) -> Tuple[np.ndarray, float, np.int32]:
    ...
    def optimize(self, num_vars: np.int32, objective_function: Callable[[np.ndarray], float],
